...
public class FlurryRodPillarProcedure {
	public static boolean execute(LevelAccessor world, double x, double y, double z, boolean push, double height) {
		...
		for (int index0 = 0; index0 < (int) height; index0++) {
			if (world.isEmptyBlock(BlockPos.containing(x, y + yOff, z)) || (world.getBlockState(BlockPos.containing(x, y + yOff, z))).is(BlockTags.create(ResourceLocation.parse("forge:amnesic_breakable")))) {
				DelayedPillarCrystalProcedure.execute(world, x, (y + yOff) - 1, z, push, yOff * 2);
				success = true;
				yOff = yOff + 1;
			}
		}
