...
public class BigGoldenCanopyProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z) {
		...
		for (int index2 = 0; index2 < (int) (rad_off * 4); index2++) {
			BlockCircleProcedure.execute(world, x, y - 2, z, EndlessEndModBlocks.BLOSSOMING_GOLDEN_SEPAL_LEAVES.get().defaultBlockState(), false, 0.005, rad_off, 1, 1);
			BlockCircleProcedure.execute(world, x, y - 2, z, EndlessEndModBlocks.GOLDEN_SEPAL_LEAVES.get().defaultBlockState(), false, 1, rad_off, 1, 1);
			rad_off = rad_off - 0.25;
		}
		rad_off = stored_rad;
		BlockCircleProcedure.execute(world, x, y + 1, z, EndlessEndModBlocks.BLOSSOMING_GOLDEN_SEPAL_LEAVES.get().defaultBlockState(), false, 0.005, rad_off, 1, 1);
