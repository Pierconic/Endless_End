...
public class BlockCircleProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, BlockState filler, boolean replace, boolean subterra, double chance, double radius, double radx, double radz) {
		double counter = 0;
		double x2 = 0;
		double z2 = 0;
		double layer = 0;
		while (counter <= 360) {
			...
			if ((!(world.getBlockState(BlockPos.containing(x + x2 + 0.5, y, z + z2 + 0.5))).isCollisionShapeFullBlock(world, BlockPos.containing(0, 0, 0)) || replace) && Math.random() < chance
					&& (world.getBlockState(BlockPos.containing(x + x2 + 0.5, y, z + z2 + 0.5)).canOcclude() || !subterra)) {
