...
public class GrowGiantGoldenStemProcedure {
	public static void execute(LevelAccessor world, double x, double y, double z, boolean subterra, double lambda) {
		...
		if (Math.random() < 0.5) {
			sz = 1;
			for (int index1 = 0; index1 < Mth.nextInt(RandomSource.create(), 4, 13); index1++) {
			...
			sx = 1;
			for (int index2 = 0; index2 < Mth.nextInt(RandomSource.create(), 4, 13); index2++) {
		...
		if (!world.canSeeSkyFromBelowWater(BlockPos.containing(x + sx * dr, y - sy1, z + sz * dr))) {
			...
			if (!grounded) {
				GoldenCanopyProcedure.execute(world, x + sx * dr, y - sy1, z + sz * dr);
			}
		}
	}
}
